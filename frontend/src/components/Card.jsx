...
const Card = ({ id, sender, sender_address, link, image, emailCount, isUnsubscribed, isDeleted, latestDate, onUnsubscribe, onDelete }) => {
    ...
    async function handleUnsubClick () {
        try{ 
            await axios.patch(`${import.meta.env.VITE_API_URL}/gmail/unsub`, {email_id: id}, { withCredentials: true })
            onUnsubscribe(id);
        } catch (err) {
    ...
    async function handleCleanInbox () {
        try{ 
            await axios.delete(`${import.meta.env.VITE_API_URL}/gmail/delete`, {data: { subscription_id: id }, withCredentials: true});
